...
protected:
	...
	void ExportTexture(std::ostringstream& maincode, const char* name, int32_t registerId)
	{
		...
		{
			maincode << "SamplerState " << name << "_texture_sampler : register(s" << registerId << ");" << std::endl;
		}
	}
							  ...
							  int32_t instanceCount)
	{
		...
		for (int stage = 0; stage < 2; stage++)
		{
			...
			for (int32_t i = 0; i < actualTextureCount; i++)
			{
				...
				//	else
				//	{
				//		baseCode = Replace(baseCode, keyP, material->GetTextureName(i)+std::string(".Sample(") + material->GetTextureName(i) + "_sampler,GetUV(");
					...
					if (stage == 0)
					{
						baseCode = Replace(baseCode,
										   ...
										   std::string(material->GetTextureName(i)) + "_texture.SampleLevel(" +
											   material->GetTextureName(i) + "_texture_sampler,GetUV(");
		...
		if (material->GetCustomData1Count() > 0)
		{
			...
			{
				/*int anum = 0;
				while (std::strstr(shaderData.CodeVS.c_str(),"ATTRIB"))
				{
					anum++;
				}*/
				shaderData.CodeVS =
					Replace(shaderData.CodeVS,
							"//$C_IN1$",
							GetType(material->GetCustomData1Count()) + " CustomData1 : ATTRIB" + std::to_string(inputSlot) + ";");
			}
		...
		if (material->GetCustomData2Count() > 0)
		{
			if (isSprite)
			{
				shaderData.CodeVS =
					Replace(shaderData.CodeVS,
							"//$C_IN2$",
							GetType(material->GetCustomData2Count()) + " CustomData2 : ATTRIB" + std::to_string(inputSlot) + ";");
			}
			shaderData.CodeVS =
				Replace(shaderData.CodeVS,
						"//$C_OUT2$",
			...
			shaderData.CodePS =
				Replace(shaderData.CodePS,
						"//$C_PIN2$",
						GetType(material->GetCustomData2Count()) + " CustomData2 : ATTRIB" + std::to_string(outputSlot) + ";");
		}
