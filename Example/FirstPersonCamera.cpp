...
void Camera::Update(InputController& Controller, float ElapsedTime)
{
    ...
    m_ViewMatrix  = float4x4::Translation(-m_Pos) * CameraRotation * SrfPreTransform;
    m_WorldMatrix = WorldRotation * float4x4::Translation(m_Pos);
    CameraViewProj = (m_ViewMatrix* m_ProjMatrix).Transpose();
}

float4x4 Camera::GetReferenceRotiation() const
{
    // clang-format off
    return float4x4
    {
...
void Camera::SetReferenceAxes(const float3& ReferenceRightAxis, const float3& ReferenceUpAxis, bool IsRightHanded)
{
...
void Camera::SetProjAttribs(Float32 NearClipPlane, Float32 FarClipPlane, Uint32 Width, Uint32 Height, Float32 FOV, SURFACE_TRANSFORM SrfPreTransform, bool IsGL)
{
